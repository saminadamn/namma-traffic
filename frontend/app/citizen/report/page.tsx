    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className={`gov-card p-4 ${isPersonnel ? "border-gov-100" : ""}`}>
            <p className="text-xs font-semibold text-gov-900 mb-3">
              {isPersonnel ? "Your report flow" : "Verification flow"}
            </p>
            {isPersonnel ? (
              <>
                <div className="mb-2 flex items-center gap-1.5 bg-gov-50 border border-gov-100 rounded-lg px-3 py-2">
                  <svg className="w-3.5 h-3.5 text-gov-500 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
                  </svg>
                  <span className="text-[10px] font-semibold text-gov-500">Traffic Personnel · Auto-verified</span>
                </div>
                <ol className="space-y-2.5">
                  {[
                    { step: "Submit your report",   sub: "Logged in as Traffic Personnel",     color: "bg-gov-100 text-gov-500" },
                    { step: "Instant verification", sub: "No officer review needed",            color: "bg-gov-100 text-gov-500" },
                    { step: "Goes live on map",     sub: "Active incident created immediately", color: "bg-gov-100 text-gov-500" },
                  ].map((item, i) => (
